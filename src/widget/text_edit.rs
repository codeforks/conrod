    ...
    fn update(self, args: widget::UpdateArgs<Self>) -> Self::Event {
                        ...
                        event::Button::Keyboard(key) => {
                            match key {
                                ...
                                input::Key::Backspace | input::Key::Delete => {
                                    let delete_word =
                                        press.modifiers.contains(input::keyboard::CTRL);
                                        ...
                                        Cursor::Idx(cursor_idx) => {
                                            ...
                                            let end = match (key, delete_word) {
                                                ...
                                                (input::Key::Backspace, true) => {
                                                    cursor_idx.previous_word_start(
                                                        &text,
                                                        line_infos,
                                                    )
                                                }
        ...
        if let Cursor::Selection { start, end } = cursor {
            ...
            if state.ids.selected_rectangles.len() < selected_rects.len() {
                ...
                state.update(|state| {
                    state.ids.selected_rectangles.resize(num_rects, id_gen)
                });
            }
