...
pub type PositionParents = std::iter::Chain<
    std::option::IntoIter<widget::Id>,
    std::option::IntoIter<widget::Id>,
>;

/// An alias for some filtered children walker.
pub type FilteredChildren = daggy::walker::Filter<
    Children,
    fn(&Graph, EdgeIndex, widget::Id) -> bool,
>;
...
impl Container {
    ...
    where
        ...
        Style: Any + 'static,
    {
        self.maybe_state.as_ref().and_then(|boxed_state| {
            boxed_state.downcast_ref()
        })
    }
...
impl Graph {
    ...
    pub fn with_node_capacity(n_nodes: usize) -> Self {
        ...
        Graph { dag: Dag::with_capacity(n_nodes, n_edges) }
    }
    ...
    pub fn edge_parent(&self, idx: widget::Id, edge: Edge) -> Option<widget::Id> {
        self.parents(idx).find(self, |g, e, _| g[e] == edge).map(
            |(_,
              n)| n,
        )
    }
    ...
    pub fn position_parents(&self, idx: widget::Id) -> PositionParents {
        self.x_position_parent(idx).into_iter().chain(
    ...
    pub fn depth_parent_recursion(
        ...
        fn depth_edge_parent(graph: &Graph, idx: widget::Id) -> Option<IndexPair> {
            graph.parents(idx).find(
                graph,
                |g, e, _| g[e] == Edge::Depth,
            )
        }
    ...
    pub fn x_position_parent_recursion(
        ...
        fn x_position_edge_parent(graph: &Graph, idx: widget::Id) -> Option<IndexPair> {
            graph.parents(idx).find(graph, |g, e, _| {
    ...
    pub fn position_children(&self, idx: widget::Id) -> PositionChildren {
        self.x_position_children(idx).chain(
            self.y_position_children(idx),
        )
    }
    ...
    where
        F: Fn(Edge) -> bool,
    {
        self.parents(child).any(self, |g, e, n| {
            n == parent && is_edge(g[e])
        })
    }
    ...
    pub fn pre_update_cache(
        ...
        let maybe_parent_id = |graph: &mut Self| match maybe_parent_id {
            Some(parent_id) => {
                match graph.node(parent_id).is_some() {
                    true => Some(parent_id),
                    false => {
                        panic!(
