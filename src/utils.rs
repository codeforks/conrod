    ...
    I: IntoIterator<Item = T>,
{
    match iter_diff(elems.iter(), new_elems.into_iter()) {
        Some(IterDiff::FirstMismatch(i, mismatch)) => Cow::Owned(
            elems[0..i]
                .iter()
                .cloned()
                .chain(mismatch)
        ...
        Some(IterDiff::Longer(remaining)) => Cow::Owned(
            elems.iter().cloned().chain(remaining).collect(),
        ),
        Some(IterDiff::Shorter(num_new_elems)) => Cow::Owned(
            elems
                .iter()
                .cloned()
                .take(num_new_elems)
