        ...
        fn next(&mut self) -> Option<Self::Item> {
            ...
            layout.next().map(|g| {
                ...
                let right = g.pixel_bounding_box()
                    ...
                    .unwrap_or_else(|| {
                        left + g.unpositioned().h_metrics().advance_width as Scalar
                    });
                *next_left = right;
...
pub mod cursor {
    ...
    impl Index {
            ...
            I: Iterator<Item = super::line::Info>,
        {
            ...
            if char > 0 {
                let new_char = char - 1;
                line_infos.nth(line).and_then(|info| if new_char <=
                    info.char_range().count()
                {
                    Some(Index {
                        line: line,
                        char: new_char,
        ...
        {
            let Index { line, char } = self;
            line_infos.nth(line).and_then(|info| if char >=
                info.char_range().count()
            {
                line_infos.next().map(|_| {
                    Index {
        ...
        I: Iterator<Item = (Xs<'a, 'a>, Range)>,
    {
        ...
        xys_per_line_enumerated.next().and_then(|(first_line_idx,
          (first_line_xs,
           first_line_y))| {
...
pub mod line {
    ...
    where
        F: for<'b> FnMut(&'b str, &'b super::Font, FontSize, Scalar)
                      -> (Break, Scalar),
    {
        type Item = Info;
        fn next(&mut self) -> Option<Self::Item> {
