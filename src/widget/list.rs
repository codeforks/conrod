...
use {color, Color, Colorable, Positionable, Scalar, Sizeable, Widget, Ui, UiCell};
use graph;
    ...
    S: ItemSize,
{
    ...
    pub fn next(&mut self, ui: &Ui) -> Option<Item<D, S>> {
        ...
        let node_index = match ui.widget_graph()
            .widget(list_id)
    ...
    S: ItemSize,
{
        ...
        W: Widget,
    {
        ...
        widget
            ...
            .and(|w| {
                D::position_item(w, last_id, scroll_trigger_id, first_item_margin)
            })
...
impl ItemSize for Fixed {
        ...
        D: Direction,
    {
        ...
        // instantiate the visible items.
        {
            ...
            scroll_trigger.color(color::TRANSPARENT).parent(id).set(
                state.ids.scroll_trigger,
                ui,
            );
        }
        ...
        let (item_idx_range, first_item_margin) = match item_instantiation {
            ...
            ItemInstantiation::OnlyVisible => {
                ...
                let end_visible_item_idx = std::cmp::min(
                    (num_start_hidden_items + num_visible_items).ceil() as
                        usize,
                    num_items,
                );
